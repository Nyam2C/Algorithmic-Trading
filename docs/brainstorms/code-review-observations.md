...
- **[P2] `open_position`과 `open_position_maker` 코드 중복**: 149~211줄 vs 213~322줄. 포지션 체크, 레버리지 설정, 사이징, position 저장 로직이 거의 동일. 공통 로직 추출 권장.
...
- **[P2] `_build_market_prompt`와 `_build_market_prompt_with_reason` 완전 중복**: 78~138줄 vs 246~306줄. `formatted_data` 구성 동일. 공통 메서드 추출 권장.
...
- **[P2] `get_pattern_insights`와 `get_worst_patterns` 코드 중복**: 649~725줄 vs 727~800줄. RSI/시간대 분석 구조 동일, 필터만 다름. 공통 메서드 추출 가능.
